 ...
 *
 * <p>
 * This class represents an active torrent on the tracker. The torrent information is kept in-memory, and is created from the byte blob one
 * would usually find in a <tt>.torrent</tt> file.
 * </p>
 *
 * <p>
...
public class Torrent {
     ...
     *
     * Parses the meta-info data (which should be B-encoded as described in the BitTorrent specification) and create a Torrent object from
     * it.
     *
     * @param torrent The meta-info byte data.
     * @param seeder Whether we'll be seeding for this torrent or not.
     *
     * @throws IOException When the info dictionary can't be read or encoded and hashed back to create the torrent's SHA-1 hash.
     */
    public Torrent(byte[] torrent, boolean seeder) throws IOException {
         ...
         *
         * <p>
         * If the torrent doesn't define an announce-list, use the mandatory announce field value as the single tracker in a single announce
         * tier. Otherwise, the announce-list must be parsed and the trackers from each tier extracted.
         * </p>
         *
     ...
     *
     * <p>
     * For a single-file torrent, this is usually the name of the file. For a multi-file torrent, this is usually the name of a top-level
    ...
    public static String byteToString(byte[] bytes) throws UnsupportedEncodingException {
        return new String(bytes, SystemDefaultProperties.BYTE_ENCODING);
    }

    /**
     * Return an hexadecimal representation of the bytes contained in the given string, following the default, expected byte encoding.
     *
     ...
     *
     * <p>
     * If the environment variable TTORRENT_HASHING_THREADS is set to an integer value greater than 0, its value will be used. Otherwise, it
     ...
     *
     * <p>
     * Hash the given file to create the {@link Torrent} object representing the Torrent metainfo about this file, needed for announcing
     ...
     *
     * <p>
     * Hash the given files to create the multi-file {@link Torrent} object representing the Torrent meta-info about them, needed for
     * announcing and/or sharing these files. Since we created the torrent, we're considering we'll be a full initial seeder for it.
     * </p>
     *
     ...
     *
     * <p>
     * Hashes the given file piece by piece using the default Torrent piece length (see {@link #PIECE_LENGTH}) and returns the concatenation
     * of these hashes, as a string.
     * </p>
     *
     * <p>
