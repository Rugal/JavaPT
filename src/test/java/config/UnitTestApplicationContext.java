...
//        bean.setBencode(torrent.getEncoded());
    ...
    @Bean
    public String credential(User user, Post post)
    {
        return BCrypt.hashpw(String.format("%d%d", user.getUid(), post.getPid()), BCrypt.gensalt());
    }

    @Autowired
    @Bean
    public String uriString(@Qualifier("credential") String credential, User user)
    {
        return "/announce?"
               + "uid=" + user.getUid()
